...
const direccionId = route.params.direccionId as string
...
const loadingDireccion = ref(true)

onMounted(async () => {
  try {
    const { data } = await http.get(`/t/${slug}/clientes/${clienteId}/direcciones/${direccionId}`)
    const direccion = data.data ?? data

    form.alias = direccion.alias ?? ''
    form.calle = direccion.calle ?? ''
    form.numero = direccion.numero ?? ''
    form.colonia = direccion.colonia ?? ''
    form.cp = direccion.cp ?? ''
    form.ciudad = direccion.ciudad ?? ''
    form.estado = direccion.estado ?? ''
    form.referencia = direccion.referencia ?? ''
    form.latitud = direccion.latitud ?? ''
    form.longitud = direccion.longitud ?? ''
    form.instrucciones_entrega = direccion.instrucciones_entrega ?? ''
  } catch {
    error.value = 'No se pudo cargar la dirección.'
  } finally {
    loadingDireccion.value = false
  }
})
...
async function onSubmit() {
  ...
  try {
    await http.put(`/t/${slug}/clientes/${clienteId}/direcciones/${direccionId}`, form)
    success.value = 'Dirección actualizada correctamente.'
    setTimeout(
    ...
    if (axios.isAxiosError(err) && err.response?.status === 422) {
      ...
      error.value = 'No se pudo actualizar la dirección, intenta de nuevo.'
    }
  } finally {
    loading.value = false
  }
...
<template>
  <TenantLayout>
    <UiCard title="Editar dirección">
      <p v-if="loadingDireccion" class="text-sm text-black/50">Cargando...</p>
