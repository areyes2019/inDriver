...
onMounted(async () => {
  try {
    const { data } = await http.get(`/t/${slug}/pedidos/recursos`)
    clientes.value = data.clientes
    despachadores.value = data.despachadores
    conductores.value = data.conductores
    vehiculos.value = data.vehiculos
  } catch {
    error.value = 'No se pudieron cargar los catálogos del formulario.'
  }
})
...
async function onSubmit() {
  ...
  try {
    await http.post(`/t/${slug}/pedidos`, { ...form, id_cliente: form.id_cliente || null })
...
<template>
  <TenantLayout>
    <UiCard title="Nuevo pedido">
      <form class="max-w-2xl space-y-5" @submit.prevent="onSubmit">
        <label class="block">
