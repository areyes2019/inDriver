...
// spec tenant/024: un pedido "lo antes posible" nace PENDIENTE y sale de la misma petición ya
// PUBLICADO. Los agendados sí se quedan PENDIENTE (ver PublicacionPedidoTest).
it('creates a pedido with an autogenerated numero_pedido, PUBLICADO estado, and logs it', function () {
    $tenant = pedidoTenant();
    $admin = pedidoUsuario($tenant);
...
it('creates a pedido with es_prueba false when the tenant is not in modo_prueba', function () {
    $tenant = pedidoTenant();
    $admin = pedidoUsuario($tenant);
    ...
    $this->actingAs($admin, 'usuario')
        ->postJson('/api/v1/t/cafe-luna/pedidos', pedidoDatosValidos())
        ->assertCreated()
        ->assertJsonPath('es_prueba', false);
});

it('snapshots es_prueba from modo_prueba at creation, unaffected by later toggles', function () {
    $tenant = pedidoTenant();
    $admin = pedidoUsuario($tenant);

    tenancy()->initialize($tenant);
    ConfiguracionTenant::establecer(ConfiguracionTenant::MODO_PRUEBA, 'Sí');
    tenancy()->end();
    ...
    tenancy()->initialize($tenant);
    ConfiguracionTenant::establecer(ConfiguracionTenant::MODO_PRUEBA, 'No');
    tenancy()->end();

    tenancy()->initialize($tenant);
    expect(Pedido::find($response->json('id_pedido'))->es_prueba)->toBeTrue();
    tenancy()->end();
});
...
it('advances a pedido through a valid transition and stamps the matching fecha', function () {
    ...
    // Agendado a propósito: es el único que sigue naciendo PENDIENTE (spec tenant/024), y por
    // tanto el único sobre el que se puede probar la transición manual a PUBLICADO.
    $creado = $this->actingAs($admin, 'usuario')
        ->postJson('/api/v1/t/cafe-luna/pedidos', pedidoDatosValidos([
            'lo_antes_posible' => false,
            'hora_desde' => '10:00',
